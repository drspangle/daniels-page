...
Most content updates happen in `index.html`. Keep related assets in `img/` or `resources/` and use relative links so the site works both locally and on GitHub Pages.
...
LaTeX source and reproducible PDF generation are not yet part of this repository. The planned next step is to add the LaTeX source in a separate branch, generate the PDF into `resources/Daniel_Smullen_CV.pdf`, and keep the public download URL unchanged.
